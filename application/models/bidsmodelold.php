...
class Bidsmodelold extends CI_Model {
		...
		*/
		...
		if ($result->num_rows()){
			...
			foreach($result->result() as $row){
				...
				$string = '<li class="reslist btn '.$class.' span12" id="r_'.$row->id.'" grp="'.$row->grp.'" subs="'.$row->con.'" conf="'.$conf.'" style="margin: 2px 0px;"  title="Щелчок добавит этот '.(($row->cat > 1) 
					? 'КОНФИДЕНЦИАЛЬНЫЙ' 
		...
		if ($result->num_rows()){
			...
			array_push($output,'<tr>
				...
				<th style="width:75px;">Дата подачи</th>
				<th style="width:75px;">Номер заявки</th>
			...
			array_push($output,'<h3 class="muted">У этого пользователя нет поданных заявок</h3>');
		}
	...
	public function papers_get(){
		...
		*/
		// очистка списка ресурсов от полученных ранее заявок 
		// на домен, интернет и электронную почту, а также Wi-Fi
		...
		*/
		...
		if($genmode == "pdf") {
			...
			$filename = 'Заявки на информационные ресурсы '.implode(array($this->input->post('name_f'), $this->input->post('name_i'), $this->input->post('name_o')),"_").'.doc';
			//print $outfile;
	...
	public function fill_template(){
		...
		// на саом деле, теперь поле адреса единое и проблем выбора между ними не будет
		...
		// простая выборка названия должности
	...
	public function turv_paper_get(){
		...
		<td  style="font-size:11pt;text-align:left;">Табель учёта рабочего времени</td>
		<td  style="font-size:11pt;text-align:center;width:17mm;"></td>
		<td  style="font-size:11pt;text-align:center;width:17mm;"></td>
		<td  style="font-size:11pt;text-align:center;width:17mm;">х</td>
		<td  style="font-size:11pt;text-align:center;width:17mm;"></td>
		</tr>
		<tr>
		...
		$templatedata['owner_staff'] = "и.о. Начальника управления";
		$templatedata['r_owner'] = "М.А. Мартынова";
		$this->resource_insert($resdata);
		$result = $this->db->query("SELECT 
		`resources_items`.id
		FROM
	...
	*/
	...
	public function reget_orders($orders){
		...
		// используется вместо continue, чтобы иметь возможность сброса более 1 заявки в бланк в текущем алгоритме (косолапо, конечно)
		$sogl = array();
		foreach($orders as $val){
