...
class Uvmr extends CI_Controller {
	...
	public function __construct() {
		...
		$this->session->set_userdata('pageHeader', 'ќтчЄты ”ћ¬–√ќ«»ијќ');
	...
	public function index($user_id=0,$page=1){
		...
		$act['content'] = "<h2>—пециальные отчЄты ќтдела по защите информации.</h2><hr>".$this->load->view('uvmr/uvmr', $data, true);
	...
	public function passport($user_id=0){
		//$this->output->enable_profiler(TRUE);
		$this->session->set_userdata('pageHeader', 'ѕаспорт рабочего места');
		$act = array();
