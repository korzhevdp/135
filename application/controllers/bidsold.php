...
class Bidsold extends CI_Controller {
	...
	*/
	public function __construct() {
		parent::__construct();
		$this->load->model('bidsmodelold');							//подключается модель заявок
	...
	public function index(){
		...
		$act['menu'] = ($this->session->userdata("base_id"))		// формируем меню
			? $this->load->view('menu/navigation', '', true)		// либо меню пользователя
			: '<div class="span2"></div>';							// либо пустое поле (пользователь "оформление заявок")
		
		$act['content'] = (in_array($this->session->userdata('admin_id'), array())) //селектор dev-режима. При нужде - добавить в массив ID операторов в array()
			? $this->load->view('bids/mainform_dev', $res, true)	// девелоперский режим
